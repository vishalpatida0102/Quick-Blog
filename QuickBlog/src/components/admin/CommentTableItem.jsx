...
const CommentTableItem = ({comment, fetchComments}) => {
    ...
    const deleteComment=async()=>
    {
        try {
            ...
            const confirm=window.confirm('Are you sure you want delete this comment ?');
            if(!confirm)return;
            ...
            {
                toast.success(data.message)
                fetchComments() //refrese the data after..
            }
            else
            {
                toast.error(data.message)
        ...
        <tr className='order-y border-gray-300'>
            ...
            <td className='px-6 py-4'>
                <div className='inline-flex items-center gap-4'>
                    {
                    !comment.isApproved ? <img onClick={approvedComment}  src={assets.tick_icon} className='w-5 hover:scale-110 transition-all cursor-pointer'/> : 
